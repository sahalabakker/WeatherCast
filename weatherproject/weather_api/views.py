...
def indexx(request):
    return render(request, "weather_api/indexx.html")

def login_index(request):
    if request.POST:
        emaill=request.POST.get("email")
        pswrdd=request.POST.get("password")
        data=Reg.objects.get(email=emaill)
        if data.email==emaill and data.pswrd==pswrdd:
             request.session['emaill']=data.email
             return redirect('home')
        else:
            return HttpResponse("Invalid") 
    return render(request, "weather_api/login_index.html") 
...
def reg(request):
    if request.POST:
        ...
        #utype=1
        #dob=request.POST["Year"]

        a=Reg.objects.create(finame=finame,laname=laname,email=email,area=area,phone=phone,pswrd=passw)
        a.save()
    return render(request, "weather_api/reg.html")       
